...
# render Organisms::Card::Record.new @user, only: [:name, :email], except: nil
module Design
  module Component
    module AtomicDesign
      module Organisms
        module Card
          class Record < Component
            ...
            renders_many :fields, lambda_slots_component_handler(AtomicDesign::Molecules::Field)
            renders_many :tables, lambda_slots_component_handler(AtomicDesign::Molecules::Table)
            renders_many :buttons, lambda_slots_component_handler(AtomicDesign::Atoms::Button)
            ...
            def call
              with_card attributes do |card|
                if content?
                  raise ArgumentError, 'Content is not allowed in this component'
                else
