...
describe AtomicDesign::Base, type: :component do
  ...
  describe '継承とモジュール連携' do
    ...
    it 'DefaultLayoutモジュールがincludeされている' do
      expect(component_class.included_modules).to include(AtomicDesign::Modules::DefaultLayout)
    end

    it 'Propertyモジュールがincludeされている' do
      expect(component_class.included_modules).to include(AtomicDesign::Modules::Property)
    end
  end
