...
describe Property::Functions::State do # :nodoc:
  SIZES = {
  ...
  it 'initialize' do
    ...
    expect { described_class.new(SIZES, name: :dummy, required: true) }.not_to raise_error
  end
  ...
  it 'method access' do
    state = described_class.new(SIZES, name: :dummy, default: :info)
    expect(state.name).to eq(:dummy)
  end
end
